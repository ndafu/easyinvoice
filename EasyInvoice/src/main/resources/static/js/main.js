...
function readURL(input) {
	...
	if(image_type!='image/png' && image_type!='image/jpg' && image_type!='image/jpeg'){
		...
		if(input.files[0].size>1000000){
			...
			//console.log('______________________'+input.files[0].type);
					...
					var articles = dta.body;
						...
						if (articles.lenght === 0) {
							...
							for (i = 0; i < articles.length; i++) {
								...
								$('#dir_inv_art_detail').append($('<td><button type="button" class="btn btn-danger deleteitems" onclick="deleteInvoiceArticle(' + articles[i].id + ')" th:data-ref="' + articles[i].id + '"><i class="fa fa-trash"> </i></button></td>'))
								$('#dir_inv_art_detail').append('</tr>');
							}
							var gdTotal = sub_total + tax;
							$('#inv_total').val(numberFormat(gdTotal));
...
$(document).ready(function() {
	...
	$('#prof_saveuser').click(function(){
		...
		if(pass!='' && pass!=pass_confirm){
			toastr.error("Pride bien vouloir confirmer votre mot de passe");
		...
		if(phone==''){
			toastr.error('Bien vouloir fournir votre numde téléphone');
		...
		if((last_name!='' && first_name!='' &&  email!='' &&  phone!='' )&& (pass=='' || (pass!='' && pass==pass_confirm))){
			$.ajax({
				...
				data:{
					...
					if(data.code=='1'){
						toastr.success("Mise jour de votre profil effectuavec succSi vous avez modifier votre mot de passe, vous devez utiliser le nouveau mot de passe lors de votre nouvelle connexion.");
						$('#prof_password_confirm').val('');
						$('#prof_password').val('');
						$('#prof_phone').val('');
						$('#prof_email').val('');
	...
	$('#saveuser').click(function(ev) {
		...
		if(organization==''){
			toastr.error("Pride bien selectionner l'entreprise employeur du nouvel utilisateur");
	...
	$('#savetax').click(function(ev) {
		...
		if (tax_title == '') {
			...
			toastr.error('Pride bien vouloir entrer le titre de cette taxe');
		} else {
		...
		if (tax_abbr == '') {
			...
			toastr.error("Pride bien vouloir indiquer l'abbreviation pour cette taxe");
		} else {
		...
		if (tax_rate == '') {
			...
			toastr.error('Pride bien vouloir entrer le taux de cette taxe');
		} else {
		...
		if (tax_rate != '' && tax_abbr != '' && tax_title != '') {
			...
			$.ajax({
				...
				data: {
					...
					if (dta.code == '1') {
						toastr.success('Enregistrement de la nouvelle taxe effectuavec succès');
						location.reload();
					}
				}
			});
	...
	$('#saveArticle').click(function(e) {
		...
		if (art_title != '' && art_unity_price != '' && art_description != '') {
			$('#art_description').attr('class', 'form-control form-control-border is-valid');
			$('#art_unity_price').attr('class', 'form-control form-control-border is-valid');
			$('#art_title').attr('class', 'form-control form-control-border is-valid');
	...
	$('#saveClient').click(function(ev) {
		...
		if (cli_tin == '') {
			...
			toastr.error('Pride fournir le Numd\'Immatriculation Fiscal (NIF) du nouveau client');
		} else {
		...
		if (cli_tin != '' && cli_name != '' && cli_telephon != '' && cli_contact_person != '') {
			$('#cli_name').attr('class', 'form-control form-control-border is-valid');
			$('#cli_tin').attr('class', 'form-control form-control-border is-valid');
			$('#cli_telephon').attr('class', 'form-control form-control-border is-valid');
			$('#cli_contact_person').attr('class', 'form-control form-control-border is-valid');
		...
		if (!com_manager && !com_head_admin && !com_head_fina && !com_log_officer && !com_hr_off && !com_cash && !com_account) {
			...
			$.ajax({
				...
				data: {
					...
					com_hr_off:com_hr_off, 
					com_cash:com_cash, 
					com_account:com_cash
				},
				success: function(dta) {
					if(dta.code=='1'){
						toastr.success("Mise jour de l'utilisateur effectuée avec succès");
						$('#edituser').modal('hide');
						location.reload();
					}
				}
		...
		if (quo_quantity != '' && quo_unity_price != '' && quo_art != '' && quo_taxe!='') {
			...
			$.ajax({
				...
				data: {
					...
					if (dta.code == '1') {
						console.log("Data unity price : " + dta.body[0].unity_price);
						var sales = dta.body;
						$('#quotation_detail').empty();
						if (sales.length === 0) {
							$('#quotation_detail').append($('<tr>'))
				...
				data: {
					...
					if (dta.code == '1') {
						...
						if (art.length === 0) {
							...
							var qrText = 'Client : '+quot.client.tin+'; Ref : '+ref+' ; Date :'+formatDate(quot.quotationDate)+' ; Total Amount : '+numberFormat(quot.grandTotal)+' ; VAT Amount : '+numberFormat(quot.totalTax)
...
//	});
			...
			if (id != '') {
				$.ajax({
					...
					data: {
						...
						if (res.code == '1') {
							console.log("Data unity price : " + res.body[0].unity_price);
							var sales = res.body;
							$('#quotation_detail').empty();
							if (sales.length === 0) {
								$('#quotation_detail').append($('<tr>'))
	...
	$('#dir_save_invoice').click(function(){
		...
		//debut control
		if(cli_name==''){
			toastr.error("Pride completer le nom du client");
				...
				data: {
					...
					if (dta.code == '1') {
						...
						if(quot.vatPayable=='2'){
							...
							$('#inv_taxable').html('EXONERE/EXEMPTED');
						}
